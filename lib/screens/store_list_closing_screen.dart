...
class ListStoreClosingPage extends StatelessWidget with AppUtilsMixin {
  ListStoreClosingPage({this.bottomTab});
  ...
  Widget build(BuildContext context) {
    return buildScaffoldWithCustomAppBarWithKey(
        ...
        AppBar(
          backgroundColor: Color.fromRGBO(16, 120, 149, 1.0),
          centerTitle: true,
          elevation: 0.0,
          title: Text(AppData().getMessage("tabClosing"),
...
class _StoreListState extends State<StoreList>
  ...
  storeList(List<Store> s) {
    ListTile makeListTile(Store store, String avatar) => ListTile(
        ...
        subtitle: buildStoreListSubTitle(
            store.isActivated, store.state, store.daySale, store.dayGain, store.lastRecordedSale),
        trailing: Container(
            width: 20,
            height: 60,
  ...
  onTapSelectedStore(Store s) {
    ...
    if (s.isActivated) {
      ...
      Navigator.of(context).pushNamed(ROUTE_PRODUCT_SALES_MANAGER);
    }
  }
